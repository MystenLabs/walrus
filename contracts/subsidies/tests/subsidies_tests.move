...
fun test_new_subsidy_object(): System {
    ...
    assert!(subsidies.storage_node_subsidy_rate() == 0);
...
fun test_new_subsidy_object_with_initial_rates_and_funds() {
    ...
    assert!(subsidies.storage_node_subsidy_rate() == initial_storage_node_subsidy_rate);
...
#[test]
fun test_set_storage_node_subsidy_rate() {
    let ctx = &mut tx_context::dummy();
    let (mut subsidies, admin_cap) = subsidies::new_for_testing(ctx);
    ...
    subsidies.set_storage_node_subsidy_rate(&admin_cap, storage_node_subsidy_rate);

    assert!(subsidies.buyer_subsidy_rate() == 0);
    assert!(subsidies.storage_node_subsidy_rate() == storage_node_subsidy_rate);
    assert!(subsidies.subsidy_pool_value() == 0);
...
#[test, expected_failure(abort_code = subsidies::EInvalidSubsidyRate)]
fun test_set_storage_node_subsidy_rate_invalid() {
    let ctx = &mut tx_context::dummy();
    let (mut subsidies, admin_cap) = subsidies::new_for_testing(ctx);

    subsidies.set_storage_node_subsidy_rate(&admin_cap, 10_001);
...
fun test_extend_blob_no_funds_no_subsidies(): (walrus::system::System, Coin<WAL>, Blob) {
    ...
    assert!(payment.value() == 250);
    assert!(subsidies.subsidy_pool_value() == 0);
    assert!(blob.storage().end_epoch() > initial_blob_storage_end);
...
fun test_extend_blob_no_funds_storage_node_subsidies(): (walrus::system::System, Coin<WAL>, Blob) {
    ...
    let mut system = system::new_for_testing();
    let (mut subsidies, admin_cap) = subsidies::new_for_testing(ctx);
    subsidies.set_storage_node_subsidy_rate(&admin_cap, 10_00); // 10%
    let mut payment = mint(1000, ctx);
...
fun test_reserve_space_no_funds_storage_node_subsidies(): (
    ...
    // it should be: payment.value() == initial_payment_value
    assert!(payment.value() == 850);
    assert!(subsidies.subsidy_pool_value() == 0);
...
fun test_reserve_space_funds_with_subsidies(): (walrus::system::System, Coin<WAL>, Storage) {
    ...
    // assert!(subsidies.subsidy_pool_value() == ?);
...
// fun setup_system_and_subsidies(
//     initial_buyer_subsidy_rate: u16,
//     initial_storage_node_subsidy_rate: u16,
//     initial_funds_value: u64,
// ): (System, subsidies::Subsidies, subsidies::AdminCap) {
//     let ctx = &mut tx_context::dummy();
...
// fun create_and_certify_blob(system: &mut System, storage: Storage): Blob {
//     let mut blob = register_default_blob(system, storage, false);
//     let certify_message = messages::certified_blob_message_for_testing(EPOCH, blob.blob_id());
//     blob.certify_with_certified_msg(system.epoch(), certify_message);
//     assert!(blob.certified_epoch().is_some());
//     blob

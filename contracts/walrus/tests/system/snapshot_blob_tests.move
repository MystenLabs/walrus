...
public fun test_snapshot_blob_certified_epoch_must_increase() {
    ...
    let mut state = snapshot_blob::create_with_empty_state();
...
#[test]
public fun test_certified_snapshot_is_stored_for_max_epochs_ahead() {
    let ctx = &mut tx_context::dummy();
    let mut system = system::new_for_testing_with_multiple_members(ctx);
    let mut nodes = test_nodes();
    set_storage_node_caps(&system, &mut nodes, ctx);
    ...
    let state = system.snapshot_blob_certification_state();
    assert!(state.get_latest_certified_epoch() == option::some(0));
    // The certified snapshot blob is stored for the system's maximum number of epochs ahead.
    let certified_events = event::events_by_type<BlobCertified>();
    assert!(certified_events.length() == 1);
    assert!(events::blob_certified_end_epoch(&certified_events[0]) == max_epochs_ahead);
    // The recorded storage end matches the certified blob's.
    assert!(state.latest_certified().map!(|s| s.end_epoch()) == option::some(max_epochs_ahead));
    nodes.destroy!(|node| node.destroy());
    system.destroy_for_testing()
}
...
public fun test_snapshot_certification_history_keeps_live_and_latest_snapshots() {
    let mut state = snapshot_blob::create_with_empty_state();
    // Snapshots stored for two epochs: the one certified in epoch E ends at E + 2 (exclusive),
    // so it has expired by the certification of epoch E + 2. One distinct snapshot per epoch for
    // epochs 0, 1, and 3; no snapshot reaches a quorum in epoch 2.
    state.certify(0, 0xA0, 2);
    state.certify(1, 0xA1, 3);
    state.certify(3, 0xA3, 5);
    ...
    assert!(state.certified_for_epoch(1).map!(|s| s.blob_id()) == option::some(0xA1));
    assert!(state.get_latest_certified_blob_id() == option::some(0xA3));
    assert!(state.certified_for_epoch(3).map!(|s| s.blob_id()) == option::some(0xA3));

...
public(package) fun certify_event_blob(
    ...
    let certified_blob_msg = messages::certified_system_blob_message(blob_id);
    blob.certify_with_certified_msg(self.epoch(), certified_blob_msg);
    self
        .event_blob_certification_state
...
/// certified, stored without payment for the system's `max_epochs_ahead`, and burned. The
/// certification state lives in a dynamic field on the `System` object because the layout of
/// `SystemStateInnerV1` is frozen, so it is detached and passed in by `system.move`.
public(package) fun certify_snapshot_blob(
    ...
    let num_shards = self.n_shards();
    // Stored for the longest lifetime the system allows: a recovering node needs the last
    // certified snapshot to still be stored, so the lifetime bounds the certification outage the
    // network can recover from (see `snapshot_blob.move`).
    let epochs_ahead = self.future_accounting.max_epochs_ahead();
    let storage = self.reserve_space_without_payment(
        encoded_blob_length(size, encoding_type, num_shards),
        0,
    ...
    // The snapshot blob is a permanent system blob like an event blob, certified by the contract
    // itself rather than by a signed message.
    let certified_blob_msg = messages::certified_system_blob_message(blob_id);
    blob.certify_with_certified_msg(self.epoch(), certified_blob_msg);

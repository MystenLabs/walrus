...
module walrus::storage_accounting {
    ...
    public(package) fun increase_storage_to_reclaim(
        accounting: &mut FutureAccounting,
        amount: u64,
    ) {
    ...
    public(package) fun rewards_to_distribute(
        accounting: &mut FutureAccounting,
    ): &mut Balance<SUI> {
    ...
    public(package) fun ring_pop_expand(
        self: &mut FutureAccountingRingBuffer,
    ): FutureAccounting {
        // Get current epoch

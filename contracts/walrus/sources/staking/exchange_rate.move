...
public(package) fun new(wal_amount: u64, shares_amount: u64): PoolExchangeRate {
    // pool_token_amount <= wal_amount as long as slashing is not implemented.
    assert!(shares_amount <= wal_amount, EInvalidRate);
    match (wal_amount) {
        0 => PoolExchangeRate::Flat,
        _ => {
            PoolExchangeRate::Variable {
                wal_amount: (wal_amount as u128),
                shares_amount: (shares_amount as u128),
            }
        },
    }
}
...
public(package) fun convert_to_wal_amount(exchange_rate: &PoolExchangeRate, amount: u64): u64 {
    match (exchange_rate) {
        ...
        PoolExchangeRate::Variable { wal_amount, shares_amount } => {
            let amount = (amount as u128);
            let res = (amount * *wal_amount) / *shares_amount;
            res as u64
        },
    }
}
...
public(package) fun convert_to_shares_amount(exchange_rate: &PoolExchangeRate, amount: u64): u64 {
    match (exchange_rate) {
        PoolExchangeRate::Flat => amount,
        PoolExchangeRate::Variable { wal_amount, shares_amount } => {
            let amount = (amount as u128);
            let res = (amount * *shares_amount) / *wal_amount;
            res as u64
        },
    }
}

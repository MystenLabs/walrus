...
public(package) fun process_pending_stake(pool: &mut StakingPool, wctx: &WalrusContext) {
    ...
    assert!(
        option::is_some(&pool.pre_active_withdrawals),
        EInvariantViolation
    );
    let mut pre_active_withdrawals = option::extract(&mut pool.pre_active_withdrawals);
...
public(package) fun wal_balance_at_epoch(pool: &StakingPool, epoch: u32): u64 {
    ...
    let pre_active_withdrawals = option::borrow(pool.pre_active_withdrawals);
...
public(package) fun is_empty(pool: &StakingPool): bool {
    assert!(
        option::is_some(&pool.pending_stake),
        EInvariantViolation
    );

...
# Install and run simulation tests
...
# Format code with unstable options (as used in CI)
...
# Format Move code (requires prettier and @mysten/prettier-plugin-move)
prettier-move --write contracts/
```
...
- The codebase uses `unwrap()` only in tests; production code uses explicit error handling with `expect` when justified
- Type conversions prefer `from/into` over `as` casts to avoid truncation
- Logging uses the `tracing` crate with structured fields and lowercase messages without periods
- Pre-commit hooks enforce code formatting and linting (install with `pre-commit install`)
...
- All public APIs must have documentation with `///` comments
- Use descriptive names; avoid abbreviations except common ones like `min`, `max`, `id`
...
- don't use info!, warn!, instead, use tracing::info(... etc
- done add this type of tracing to the tests:  === PHASE 1: Write data BEFORE starting indexer ===, use normal comments, with a period at the end.

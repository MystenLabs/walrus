...
impl<T: ThreadMode> OptimisticTransactionDB<T> {
    ...
    /// Opens a database with the given database options and column family descriptors,
    /// using the specified OCC validation policy.
    pub fn open_cf_descriptors_with_validation<P, I>(
        opts: &Options,
        path: P,
        cfs: I,
        validate_policy: OccValidationPolicy,
    ) -> Result<Self, Error>
    where
        P: AsRef<Path>,
        I: IntoIterator<Item = ColumnFamilyDescriptor>,
    {
        Self::open_cf_descriptors_internal_with_validation(opts, path, cfs, Some(validate_policy))
    }
    ...
    where
        ...
        I: IntoIterator<Item = ColumnFamilyDescriptor>,
    {
        Self::open_cf_descriptors_internal_with_validation(opts, path, cfs, None)
    }

    fn open_cf_descriptors_internal_with_validation<P, I>(
        opts: &Options,
        path: P,
        cfs: I,
        validate_policy: Option<OccValidationPolicy>,
    ) -> Result<Self, Error>
    where
        P: AsRef<Path>,
        I: IntoIterator<Item = ColumnFamilyDescriptor>,
    ...
    fn open_cf_raw(
        ...
        unsafe {
            let db = match validate_policy {
                Some(policy) => ffi_try!(
                    ffi::rocksdb_optimistictransactiondb_open_column_families_with_validate_policy(
                        opts.inner,
                        cpath.as_ptr(),
                        cfs_v.len() as c_int,
                        cfnames.as_ptr(),
                ...
                None => ffi_try!(ffi::rocksdb_optimistictransactiondb_open_column_families(
                    opts.inner,
                    cpath.as_ptr(),
                    cfs_v.len() as c_int,
                    cfnames.as_ptr(),
    ...
    pub fn write(&self, batch: WriteBatchWithTransaction<true>) -> Result<(), Error> {
        let option = WriteOptions::default();
        //option.set_sync(true);
        self.write_opt(batch, &option)
    }

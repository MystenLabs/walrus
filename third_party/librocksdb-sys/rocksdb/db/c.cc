...
rocksdb_optimistictransactiondb_open_column_families(
    ...
    const rocksdb_options_t* const* column_family_options,
    rocksdb_column_family_handle_t** column_family_handles, char** errptr) {
  return rocksdb_optimistictransactiondb_open_column_families_with_validate_policy(
      db_options, name, num_column_families, column_family_names,
      column_family_options, column_family_handles,
      static_cast<unsigned char>(
          ROCKSDB_NAMESPACE::OccValidationPolicy::kValidateParallel),
      errptr);
}

rocksdb_optimistictransactiondb_t*
rocksdb_optimistictransactiondb_open_column_families_with_validate_policy(
    const rocksdb_options_t* db_options, const char* name,
    int num_column_families, const char* const* column_family_names,
    const rocksdb_options_t* const* column_family_options,
    rocksdb_column_family_handle_t** column_family_handles,
    unsigned char validate_policy, char** errptr) {
  std::vector<ColumnFamilyDescriptor> column_families;
  for (int i = 0; i < num_column_families; i++) {
    column_families.push_back(ColumnFamilyDescriptor(
        std::string(column_family_names[i]),
  ...
  OptimisticTransactionDB* otxn_db;
  std::vector<ColumnFamilyHandle*> handles;
  ROCKSDB_NAMESPACE::OptimisticTransactionDBOptions occ_options;
  occ_options.validate_policy =
      validate_policy == static_cast<unsigned char>(
                             ROCKSDB_NAMESPACE::OccValidationPolicy::kValidateSerial)
          ? ROCKSDB_NAMESPACE::OccValidationPolicy::kValidateSerial
          : ROCKSDB_NAMESPACE::OccValidationPolicy::kValidateParallel;
  if (SaveError(errptr, OptimisticTransactionDB::Open(
                            DBOptions(db_options->rep), occ_options,
                            std::string(name),

...
binary](/usage/setup.mdx#installation). If you have multiple versions in different locations, find the
...
[install](/usage/setup.mdx#installation) the `ubuntu-x86_64-generic` version instead, which is compiled
...
[configuration](/usage/setup.mdx#configuration).
...
this case, update your configuration file with the latest [configuration](/usage/setup.mdx#configuration)

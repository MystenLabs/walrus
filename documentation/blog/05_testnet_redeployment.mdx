...
in the [setup section](/usage/setup).
...
- The `walrus` CLI now supports creating, funding, and extending *shared blobs* using the `walrus
  ...
  example of collectively managed and funded blobs. See the [shared blobs section](/usage/client-cli#shared-blobs) for more details.
...
instance. You need to request new WAL tokens through the [Testnet WAL faucet](/usage/setup#testnet-wal-faucet).
...
use the latest version of the configuration files, see the [configuration section](/usage/setup#configuration).
...
also the aggregator and publisher. Please refer to the section on the [HTTP API](/usage/web-api) for further details.

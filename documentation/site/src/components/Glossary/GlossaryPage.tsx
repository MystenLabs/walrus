...
export default function GlossaryPage() {
  ...
  const {sortedTerms, termsByLetter, availableLetters} = useMemo(() => {
    if (!map) return {sortedTerms: [], termsByLetter: {}, availableLetters: new Set()};
  ...
  if (!map) {
    return <div className="text-center py-8 text-wal-gray-80 dark:text-wal-white-60 italic">Loading glossary...</div>;
  }

  if (sortedTerms.length === 0) {
    return <div className="text-center py-8 text-wal-gray-80 dark:text-wal-white-60 italic">No glossary terms found.</div>;
  }
    ...
    <div className="flex flex-col gap-8">
        ...
        return (
            ...
            <div className="flex flex-col gap-4">
                ...
                <div key={term.label} className="border-b border-wal-gray-50 dark:border-wal-white-30 pb-4 last:border-b-0 last:pb-0">
                    <dt className="text-xl font-semibold text-wal-gray-80 dark:text-wal-white-60 mb-2">{term.label}</dt>
                    <dd className="m-0 leading-relaxed text-wal-gray-80 dark:text-wal-white-60">{term.definition}</dd>
                </div>
                ))}
            </div>
            </div>

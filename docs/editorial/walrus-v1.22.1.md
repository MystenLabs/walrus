---
title: Walrus v1.22.1
description: Reduces SUI gas costs for simultaneous blob registration, surfaces storage node checkpoint progress in the health command, and fixes default-location...
keywords: ["walrus", "release notes", "changelog", "mainnet", "storage node", "checkpoint", "blob"]
---

**Mainnet** | April 25, 2025

Reduces SUI gas costs for simultaneous blob registration, surfaces storage node checkpoint progress in the `health` command, and fixes default-location Sui wallet creation. Operators must remove the `experimental_batch_symbol_recovery` config field.

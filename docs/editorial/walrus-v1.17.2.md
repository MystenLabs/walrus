---
title: Walrus v1.17.2
description: Adds --network-key-path and --certificate-path options to the setup command, removes legacy event-provider flags, and adjusts...
keywords: ["walrus", "release notes", "changelog", "testnet"]
---

**Testnet** | March 15, 2025

Adds `--network-key-path` and `--certificate-path` options to the `setup` command, removes legacy
event-provider flags, and adjusts default storage prices and commission rates.

...
in the [setup section](/getting-started/advanced-setup).
...
- The `walrus` CLI now supports creating, funding, and extending *shared blobs* using the `walrus
  ...
  example of collectively managed and funded blobs. See the [shared blobs section](/walrus-client/storing-blobs.mdx#shared-blobs) for more details.
...
instance. You need to request new WAL tokens through the [Testnet WAL faucet](/system-overview/available-networks#testnet-wal-faucet).
...
use the latest version of the configuration files, see the [configuration section](/getting-started/advanced-setup#configuration).
...
the [JSON API](/walrus-client/json-mode.mdx)).
...
also the aggregator and publisher. Please refer to the section on the [HTTP API](/http-api/storing-blobs) for further details.

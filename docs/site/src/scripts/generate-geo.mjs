...
function getArchetype(relPath) {
  ...
  if (relPath.startsWith('typescript-sdk/') || relPath.startsWith('typescript-sdk.mdx')) return 'sdk';
...
function generateQuestions(title, description, headings, relPath) {
  ...
  switch (archetype) {
    case 'onboarding':
      ...
      break;
    case 'example':
      questions.push(`How do I build a ${topicLower} on Walrus?`);
      // Avoid "pattern pattern"
      const patternName = topicLower.replace(/\s+pattern$/i, '');
      questions.push(`What is the ${patternName} pattern in Walrus?`);
      break;
    case 'operator':
      questions.push(`How do I ${actionForm} for Walrus?`);
      break;
    case 'sdk': {
    ...
    default: // guide
      ...
      if (/^(build|create|set up|configure|install|deploy|write|test|debug|connect|query|emit|use|optimiz|integrat|run|monitor|sign|send)/i.test(actionForm)) {
        ...
        questions.push(`What is ${topicName} in Walrus?`);
        questions.push(`How does ${topicLower} work on Walrus?`);
      }
  }

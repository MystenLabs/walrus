  ...
  for (const req of goal.requires) {
    ...
    if (req.pattern !== undefined && req.min !== undefined) {
      ...
      const junkPatterns = [
        ...
        /How do I (the|a|an|walrus|set up walrus|operator|data) /i, // bad starts
        ...
        /^What is [A-Z][a-z]+ a [A-Z]/i,                          // "What is Build a Custom Indexer?"
        /(in|on) Walrus (in|on) Walrus/i,                           // "What is X on Walrus in Walrus?"
        /\bon walrus\b/,                                            // lowercase "on walrus" is always a template artifact (proper noun is "Walrus")
      ];
      let junkCount = 0;
      if (has) {

...
//
// Generates site/src/data/skills.json from the MystenLabs/walrus-skills repository.
//
// This runs at build time (wired into the `prebuild` and `prestart` scripts in
// package.json). When a skill is added, removed, or edited in the walrus-skills
// repo, the next docs build regenerates the /skills page automatically. No manual
...
const REPO_NAME = "walrus-skills";
const REPO_BRANCH = "main";
const OUT_PATH = path.join(__dirname, "../data/skills.json");

const token = process.env.GITHUB_TOKEN || process.env.GH_TOKEN;
const headers = {
  Accept: "application/vnd.github+json",
  "User-Agent": "walrus-docs-skills-generator",
...
main().catch((err) => {
  console.warn(`Skill generation skipped: ${err.message}`);

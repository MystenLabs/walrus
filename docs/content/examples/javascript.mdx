...
Reading by blob ID returns `application/octet-stream`, so set the media type yourself when you embed a blob in a page. To get stored headers such as `content-type` back, read by Sui object ID instead with `/v1/blobs/by-object-id/{blob_object_id}`. See [Reading Blobs](/docs/http-api/reading-blobs).

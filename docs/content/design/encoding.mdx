...
- **Highly efficient:** Walrus can resconstruct a blob from just one-third of the encoded symbols.
...
This detailed encoding setup results in an expansion of the blob size by a factor of \(4.5 \sim 5\). This is independent of the number of shards and the number of storage nodes.
...
In most cases, the default consistency check is sufficient, particularly when the blob writer is trusted. The strict consistency check is only needed for specific availability guarantees. When the writer is known and trusted, consistency checks can be disable entirely.

### Default consistency check

The default check balances performance with security by only verifying the read data.
...
The first 334 primary slivers contain the unencoded data (potentially padded). This means the cryptographic hashes of these slivers, combined with the blob length, uniquely determine the blob content:

1. Recompute sliver hashes for the first 334 slivers.
2. Check recomputed hashes against the metadata.
3. If hashes match, provide data to user.
4. If hashes don't match, return an error.
...
This check is strictly stronger than the default check and provides the same data consistency property. It ensures the original writer encoded the blob correctly, eliminating the possibility of inconsistent reads across different clients or time periods.
...
For quilt patches, only a variant of the default consistency check is available, because clients read only part of the quilt rather than the entire structure.
...
- **Storage:** Users store new blobs and pay [required costs](/system-overview/storage-costs) for [write](/system-overview/operations#store) and [non-best-effort read](/system-overview/operations#read) operations.
- **Availability:** Users can [prove a blob's availability to third parties](/system-overview/operations#certify-availability) without the cost of transmitting the full blob.
...
**Storage nodes** manage the actual data storage on Walrus. Each storage node holds one or more **shards** during a [storage epoch](/blog/04_testnet_update). Each blob undergoes [erasure encoding](/design/encoding), which splits it into many **slivers**. These slivers from each stored blob become distributed across all shards in the system. At any given storage epoch, a storage node associates with one or more shards. The node stores all slivers belonging to its assigned shards and serves them upon request.

...
- **Blockchain coordination:** [Sui blockchain](https://github.com/MystenLabs/sui) that manages payments, metadata, and system orchestration.

## Users

**Users** interact with Walrus through **clients** to store and read **blobs**, which are identified by their **blob ID**. Users engage with the system in two primary ways:

- **Storage:** Users store new blobs and pay [required costs](/dev-guide/costs.mdx) for [write](/dev-guide/dev-operations.mdx#store) and [non-best-effort read](/dev-guide/dev-operations.mdx#read) operations.
- **Availability:** Users can [prove a blob's availability to third parties](/dev-guide/dev-operations.mdx#certify-availability) without the cost of transmitting the full blob.
...
**Storage nodes** manage the actual data storage in Walrus. Each storage node holds one or more **shards** during a **storage epoch**. Each blob undergoes erasure encoding, which splits it into many **slivers**. These slivers from each stored blob become distributed across all shards in the system. At any given storage epoch, a storage node associates with one or more shards. The node stores all slivers belonging to its assigned shards and serves them upon request.

## Blockchain coordination

All clients and storage nodes run a Sui blockchain client, which provides the coordination layer for the entire system. The Sui chain manages several operations, including:
...
Users interact with the Sui chain to acquire storage resources and submit certificates for stored blobs. Storage nodes monitor blockchain events to coordinate their operations and respond to system changes.

### Byzantine fault tolerance

A Sui smart contract controls how shards are assigned to storage nodes. These assignments occur within **storage epochs**, which last 2 weeks on Mainnet.

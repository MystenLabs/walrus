...
Walrus does not encrypt data by default. Anyone with a blob ID can read the bytes. For private data, encrypt before upload using [Seal](/docs/data-security#seal-data-confidentiality-and-access-control). See [Encrypt before you store with Seal](/docs/typescript-sdk/seal-encryption-tutorial) for an end-to-end example.

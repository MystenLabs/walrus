...
<AgentPrompt
  prompt="Store report.pdf on Testnet for 53 epochs as a permanent blob using curl, then read it back to out.pdf. Explain how to distinguish a first-time store from one where the blob already existed."
/>
...
$ curl -X GET "$PUBLISHER/v1/blobs?epochs=5" --upload-file "some/file"
```
...
$ curl -X GET "$PUBLISHER/v1/blobs?deletable=true" --upload-file "some/file"
```

```sh
# Store file `some/file` as a permanent blob:
$ curl -X GET "$PUBLISHER/v1/blobs?permanent=true" --upload-file "some/file"
```
...
$ curl -X GET "$PUBLISHER/v1/blobs?send_object_to=$ADDRESS" --upload-file "some/file"
```

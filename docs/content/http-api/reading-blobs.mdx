...
keywords:
  ...
  - consistency check
goal:
  description: Reader can look up reading blobs details
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Reading Blobs?
  - >-
    Read blobs from Walrus using HTTP GET requests with blob IDs or Sui object
    IDs, including content type headers and consistency check options.?
  - How does reading blobs work?
answer: >-
  You can read blobs using HTTP GET requests with their blob ID or object ID.
  Set `$AGGREGATOR` to an aggregator endpoint from the [Network
  Reference](/docs/network-reference#aggregators-and-publishers).
---

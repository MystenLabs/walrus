...
Walrus supports storing and retrieving multiple blobs as a single unit called a [quilt](/docs/system-overview/quilt.mdx). Publishers and aggregators both support quilt operations.
...
Identifiers must be unique within a quilt and cannot start with `_`. The field name `_metadata` is reserved for Walrus native metadata and does not conflict with user-defined identifiers. See the [Quilt documentation](/docs/system-overview/quilt.mdx) for complete identifier restrictions.
...
You can obtain `QuiltPatchIds` from the store quilt output or by using the [`list-patches-in-quilt`](/docs/walrus-client/storing-blobs.mdx#batch-store) CLI command.

...
Walrus defines a number of [objects and smart contracts](/system-overview/core-concepts) on Sui:

- A shared [system object](/design/operations-sui) records and manages the current committee of [storage nodes](/design/architecture).

- [Storage resources](/design/operations-sui) represent empty storage space that you can use to store blobs.

- [Blob resources](/design/operations-off-chain) represent blobs being registered and [certified](/design/operations-sui) as stored.

- Changes to these objects emit [Walrus-related events](/system-overview/core-concepts#events).
...
- [Aggregator services](/design/architecture) that allow reading blobs through HTTP requests.

- [Publisher services](/design/architecture) used to store blobs to Walrus.

- A set of [storage nodes](/design/encoding) that store encoded blobs. These nodes form the decentralized storage infrastructure of Walrus.

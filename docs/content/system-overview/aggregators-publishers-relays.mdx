...
Read a blob by blob ID at `/v1/blobs/{blob_id}`, or by Sui object ID at `/v1/blobs/by-object-id/<objectId>`. Reading by object ID also returns the stored attributes as HTTP headers, so prefer it when the content type matters. See [Reading Blobs](/docs/http-api/reading-blobs).

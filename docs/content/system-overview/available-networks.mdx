...
Many other parameters, including the system capacity and prices, are dynamic. Such parameters are stored in the [system object](/system-overview/core-concepts.mdx#system-and-staking-information) and can be viewed with tools like the [Walruscan explorer](https://walruscan.com/).
...
The configuration file described on the [setup page](/getting-started/index.mdx) includes both Mainnet and Testnet configuration.
...
Also see the [Testnet terms of service](/legal/testnet_tos.mdx) under which this Testnet is made available.
...
- [x] Download and install the [Sui CLI](/getting-started/index.mdx#step-1-install-tooling).

- [x] Create a [Sui account](/getting-started/index.mdx#step-3-understanding-your-sui-account).

- [x] Obtain [Testnet SUI](/getting-started/index.mdx#step-4-fund-sui-account-with-tokens).
...
The configuration parameters for the Walrus Testnet are included in the configuration file described on the [getting started guide](/getting-started/index.mdx#step-2-configure-tooling-for-walrus-testnet). If you want only the Testnet configuration, you can get the Testnet-only configuration file. The parameters are:
...
The script generates configuration that you can use when [running the Walrus client](/docs/walrus-client/storing-blobs.mdx) and prints the path to that configuration file.

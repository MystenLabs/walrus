...
The steps involved in a store operation can be executed by the binary client or a publisher that accepts and publishes blobs through [HTTP](/http-api/storing-blobs.mdx).
...
The maximum blob size can be queried through the [`walrus info`](/walrus-client/storing-blobs.mdx#walrus-system-information) CLI command. The maximum blob size is currently 13.3&nbsp;GB. You can store larger blobs by splitting them into smaller chunks.

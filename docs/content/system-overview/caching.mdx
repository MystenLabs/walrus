...
Key the cache on the request path, which already includes the blob ID (`/v1/blobs`) or object ID (`/v1/blobs/by-object-id/<OBJECT_ID>`). The CDN then serves repeat reads of a hot blob from its edge without contacting the aggregator.

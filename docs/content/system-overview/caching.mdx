...
A blob ID is derived from the blob's contents. Reading the same blob ID always returns the same data, so a cache entry keyed by blob ID can be treated as immutable, but its lifetime is bounded by the blob's storage period and deletion risk. This is different from mutable object storage, where a key can be overwritten and caches must revalidate.

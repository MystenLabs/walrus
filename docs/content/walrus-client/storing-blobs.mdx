...
- If the blob is already certified on Walrus and is stored for a sufficient number of epochs as a permanent blob, the command skips sending encoded blob data to the storage nodes and just collects the availability certificate. However, immutable blobs derived from their content cannot be re-stored with a different ID.

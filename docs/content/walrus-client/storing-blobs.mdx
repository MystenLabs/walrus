...
Use the command-line interface (CLI) to interact with the Walrus client. The CLI is available through installing the `walrus` binary. See the [getting started documentation](/docs/getting-started/index) for prerequisites, installation, and configuration.

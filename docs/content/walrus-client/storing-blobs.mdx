...
Use the command-line interface (CLI) to interact with the Walrus client. The CLI is available through installing the `walrus` binary. See the [getting started documentation](/getting-started/index) for prerequisites, installation, and configuration.
...
All blobs stored in Walrus are public and discoverable by all. To store sensitive data, use [Seal](/data-security#seal-data-confidentially-and-access-control) or [Nautilus](/data-security#nautilus-secure-and-verifiable-off-chain-computation) to encrypt the data before storing it on Walrus.
...
Asset management on-chain still happens on the client. The upload relay just takes the unencoded blob, encodes it, and sends the slivers to the storage nodes, before returning the certificate. See in-depth details in the [Walrus upload relay](/operator-guide/upload-relay) documentation.
...
View technical details on how the tip is [computed and paid](/operator-guide/upload-relay).

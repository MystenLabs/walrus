...
<AgentPrompt
  prompt="I have a blob that expires in two epochs. Extend its storage by 53 epochs, set its content-type attribute to application/pdf, then make it a shared blob so anyone can extend it in the future."
/>

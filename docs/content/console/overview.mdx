...
**Memory** refers to Walrus Memory namespaces, the portable memory layer for AI agents. At GA you can browse, search, and manage your agent memory directly in Console, without touching the SDK. Renewing memory storage in Console is planned for a post-GA release.
...
Console keeps a perpetual free tier so new developers are not paywalled. A free storage cap, tentatively 5 GB, is planned pending analysis of real Mainnet usage. Usage-based billing for reads and egress, along with a paid top-up path, follows after GA. Console manages WAL token handling on your behalf.

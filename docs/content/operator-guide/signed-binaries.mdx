...
# Verify the binary
cosign verify-blob \
  --keys gcpkms://projects/walrus-infra/locations/global/keyRings/walrus-signing/cryptoKeys/release-sign \
  --signature walrus.sig \
  --insecure-ignore-tlog \
  walrus
```
...
# Verify the image
cosign verify --keys walrus-signing.pub mysten/walrus-service:v1.0.0-signed
```

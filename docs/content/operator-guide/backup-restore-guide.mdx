...
The following commands assume `walrus-node` is in your system's PATH. If it's not, replace `walrus-node` with the full path to the binary, for example `/opt/walrus/bin/walrus-node`.

...
```sh
sui client --n-clients.config /opt/walrus/config/sui_config.yaml objects \
...
sui client transfer --to $NEW_ADDRESS --object-ids $CAP_OBJECT_ID
```
...
```sh
sui client --n-clients.config /opt/walrus/config/sui_config.yaml \
    transfer --to $NEW_ADDRESS --object-ids $CAP_OBJECT_ID
```

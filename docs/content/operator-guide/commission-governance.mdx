...
keywords: ["walrus", "commission", "governance", "storage node", "authorization", "contract upgrades", "voting", "node operations"]
---
...
It is possible to either designate an arbitrary object as a capability for governance and commission operations or to designate an address to be authorized.

:::warning

Once set, only the authorized entity (either based on the address or capability) can change the authorization again, so only set it to addresses or objects that you control and make sure they remain accessible.
...
The following assumes that you have the `walrus` binary [correctly set up](/usage/setup.mdx), using the wallet that is currently authorized to perform these operations. If this is the first time updating the authorized entities, this is the wallet that you used to setup the storage node. To specify a wallet and config that are not in the standard locations, you can specify them using the `--wallet` and `--config` command line arguments.

The authorized entities for commission and governance are independent, they do not need to be set to the same address or object.
```sh

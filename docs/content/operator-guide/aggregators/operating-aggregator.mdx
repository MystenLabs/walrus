...
description: How to run a Walrus aggregator daemon, download client configuration, configure a systemd service, and serve large files using the blob concatenation endpoint.
keywords:
  - walrus
  - aggregator
  - daemon
...
The aggregator requires a client configuration file. If you run the aggregator on the same host as a [storage node](/docs/operator-guide/storage-nodes/storage-node-setup#binaries), the configuration is already available at `/opt/walrus/config/client_config.yaml`. Otherwise, download it (the `/opt/walrus` paths used throughout this guide are the storage node convention; substitute the directories used by your own installation):

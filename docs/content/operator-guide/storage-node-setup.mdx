...
<summary>Initial system setup instructions </summary>
    ...
    Open port **9185** (storage node) and **80** (automatic certificate renewal). If you plan to run
    an aggregator or publisher without a reverse proxy, also open **9000** (aggregator) and/or
    **9001** (publisher). If using a reverse proxy, open **443** instead:

        ```sh
        for PORT in 80 443 9185 9000 9001; do
...
You can use the [Sui Testnet faucet](https://faucet.sui.io) to obtain test SUI. Note that the faucet
has rate limits, so you may need to make multiple requests or wait between attempts. Alternatively,
transfer SUI from an existing Testnet wallet.
...
The storage node serves HTTPS only (it handles TLS directly). When checking from localhost, use

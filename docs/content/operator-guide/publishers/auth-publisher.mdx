...
The authenticated publisher enables web apps to let users upload files to Walrus without requiring them to have a wallet, as the publisher wallet covers the transaction costs. The following example demonstrates a typical workflow where users authenticate through the web app using credentials like username and password rather than a blockchain wallet.

...
```sh
/opt/walrus/bin/walrus --config /opt/walrus/config/publisher_client_config.yaml get-wal --amount 5000000000
```
...
Update the `ExecStart` line in the publisher systemd service to reference `publisher_client_config.yaml` accordingly. Make sure to [fund the publisher wallet](#fund-publisher) before starting the service.

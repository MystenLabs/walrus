...
The `site-builder` will automatically be installed in `$HOME/.local/bin`. You can run it by typing:
...
The `site-builder` client binary is currently provided for Ubuntu, macOS (Apple and Intel CPUs), and [Windows](#windows).
...
Move the `site-builder` binary to any directory included in your `$PATH` environment variable. Standard locations are `/usr/local/bin/`, `$HOME/bin/`, or `$HOME/.local/bin/`.

```sh
$ echo $PATH | tr ":" "\n"
```

You can now run `site-builder` by typing it in your terminal.
...
Similar to UNIX-based installation, you can use Windows PowerShell to download the latest build from the GCS bucket. 
...
If the configuration file is not in one of these default locations, you will likely see the following error when attempting to run any `site-builder` command:

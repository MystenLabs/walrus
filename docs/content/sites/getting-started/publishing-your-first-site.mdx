...
The Walrus Sites `site-builder` uploads your website's directory to [Walrus](https://www.walrus.xyz/), where your site resources (HTML, CSS, JavaScript, and images) are stored as blobs. A [Sui smart contract](https://docs.sui.io/guides/developer/packages/move-package-management) acts as an on-chain index, mapping each resource path to its corresponding Walrus blob ID and recording site ownership. [Portals](/docs/sites/portals/deploy-locally) use this on-chain metadata to resolve and serve your site to visitors. 

Similar to a traditional website, your directory needs an `index.html` file at the root to serve as the entry point. For example, the `walrus-snake` game you will deploy in this guide has an `index.html` with the following assets:
...
The prerequisite setup guides install the Walrus and `site-builder` binaries and configuration files to their [default directories](/docs/sites/getting-started/installing-the-site-builder#configuration). If `site-builder` commands return the following error, use the `--config` flag to specify a custom configuration path.
...
- **Mainnet sites**: Can be accessed through any Mainnet portal. To access your Walrus Site through https://wal.app, a [SuiNS name](/docs/sites/custom-domains/setting-a-suins-name) is required.

- **Testnet sites**: Can be accessed through a [self-hosted or local portal](/docs/sites/portals/deploy-locally). There are no public Testnet portals.
...
- For each file in the folder, a new site resource was created with a respective `QuiltPatchID`, which together form a [quilt](/docs/system-overview/quilt). 
...
If you are building a Walrus Site using a web framework (such as React, Node.js, or Django), ensure you rebuild your website before updating the corresponding Walrus Site.

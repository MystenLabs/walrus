...
If you are building a Walrus Site using a web framework (such as React, Node.js, or Django), ensure you point the `site-builder` to your site's build folder when running the `deploy` command.

For example, if you are using a React framework such as Vite, your directory structure should resemble the following:
...
Running `npm run build` generates a `build` folder in your project root. Note that the folder name may vary by framework (Vite calls it `dist`).
...
Now you can publish your website using the `site-builder`.

...
The configuration file is expected to be in one of the [default locations](/docs/getting-started/advanced-setup.mdx#config-custom-path), however it is possible to point elsewhere by passing the `--config` flag to the `site-builder` command. For your first run, it should be sufficient to call the `site-builder` without specifying the configuration file explicitly, which is already configured if you followed thoroughly the [installation steps](/sites/getting-started/installing-the-site-builder.mdx#configuration).

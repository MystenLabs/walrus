...
When you request a resource, a Walrus Sites [portal](/sites/portals/deploy-locally) checks that the hash of the data received from the Walrus storage and the Walrus aggregator matches the hash stored on Sui. If the hashes do not match, the portal returns the following warning page:

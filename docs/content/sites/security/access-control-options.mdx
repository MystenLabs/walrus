...
keywords:
  ...
  - data security
goal:
  description: Reader understands access control options and can apply the concepts
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  ...
  - Overview of access control for Walrus Sites.?
  - How does access control options work?
answer: >-
  All blobs stored on Walrus are public by default. Anyone who knows a blob ID
  can fetch its contents directly from a Walrus aggregator. Access control for
  Walrus Sites is not enforced at the storage layer.
---

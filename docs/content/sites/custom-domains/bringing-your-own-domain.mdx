...
description: "Complete guide to configuring a custom DNS domain for a Walrus Site by deploying a local portal and pointing your domain to it."
keywords: ["walrus sites", "custom domain", "dns configuration", "portal setup", "domain registration", "byod", "hosting", "bring your own domain"]
---
...
This guide requires you to deploy and run your own portal. The public `wal.app` portal does not support custom DNS domains. For an alternative approach that does not require running your own portal, see [Setting a SuiNS Name](./setting-a-suins-name).
...
- [x] A deployed Walrus Site with a known object ID (in base36)
- [x] A custom domain registered through a domain registrar
- [x] A server or hosting environment where you can run the Walrus Sites portal (with a public IP address)
- [x] Access to your domain's DNS settings
- [x] The Walrus Sites portal repository cloned and set up locally, see [Deploy a Local Portal](/docs/sites/portals/deploy-locally)
</TabItem>
</Tabs>
</div>
...
Environment variables are still supported for backward compatibility and override any value set in `portal-config.yaml`. The `.env.mainnet.example` and `.env.testnet.example` files in `portal/server/` document the available env var names if you need them.
...
Once your portal is running and accessible at a public IP address, configure your domain's DNS to point to it. See [Configure DNS for a Walrus Site](./configure-dns) for a full guide on adding DNS records, pointing subdomains, verifying propagation, and troubleshooting.

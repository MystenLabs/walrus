...
description: "How to prevent duplicate content SEO issues when your Walrus Site is accessible under multiple hostnames, including canonical hints and portal configuration options."
...
The standard way to tell search engines which URL is authoritative is the canonical hint. You can set it as an HTML tag in your page `<head>`, or as an HTTP response header.
...
- Keep Base36 enabled but add a canonical hint in your site HTML pointing to your preferred host, and optionally return an `X-Robots-Tag: noindex` response header for requests arriving on Base36 subdomains.

...
Any CI/CD platform that provides Linux runners can deploy Walrus Sites using the same pattern. The examples pin `ubuntu:24.04`, but any Ubuntu-based Linux image works, including earlier Ubuntu LTS releases. The following table maps the concepts from the 3 examples above to their equivalents on other platforms:

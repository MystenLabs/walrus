...
description: "How to export a Sui wallet key, format it correctly, fund your deployment address, and store credentials as GitHub secrets and variables for automated Walrus Sites deployments."
keywords: ["walrus sites", "github secrets", "deployment credentials", "sui wallet", "private key", "SUI_KEYSTORE", "SUI_ADDRESS", "github actions", "ci/cd"]
---
...
Run the following command to look up the Sui address that corresponds to a key in your keystore:
...
Before any workflow can deploy a site, the address needs 2 types of tokens:

- **SUI tokens** cover network gas fees for every transaction.
- **WAL tokens** pay for storing your site's data on Walrus.

<div className="outlined-tabs">
<Tabs>
<TabItem label="Testnet" value="testnet">

**SUI tokens:** Visit the [official Sui faucet](https://faucet.sui.io/) and request free Testnet SUI for your deployment address.

**WAL tokens:** Exchange Testnet SUI for Testnet WAL at a 1:1 rate using one of the following methods:

- Run the Walrus CLI command: `walrus get-wal`
- Visit [stake-wal.wal.app](https://stake-wal.wal.app/?network=testnet), set the network to **Testnet**, and select **Get WAL**.

</TabItem>
<TabItem label="Mainnet" value="mainnet">

For Mainnet, acquire both SUI and WAL tokens from a supported exchange and transfer them to your deployment address. You can also use Slush Wallet for token swaps to WAL or an on-ramp service. Availability varies by region.

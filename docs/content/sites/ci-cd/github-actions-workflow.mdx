...
description: "How to write and configure a GitHub Actions workflow for automated Walrus Sites deployment, including build steps, action inputs, ws-resources.json tracking, and troubleshooting."
keywords: ["walrus sites", "github actions", "workflow", "deployment", "automation", "ci/cd", "static files", "build process", "ws-resources.json"]
---

This guide explains how to create a GitHub Actions workflow that automatically deploys your Walrus Site whenever you push to your repository. The workflow uses the official [Deploy Walrus Site action](https://github.com/MystenLabs/walrus-sites-github-actions) maintained by Mysten Labs.
...
Choose exactly 1 of the following 3 authentication methods. Providing more than 1 causes the action to fail.

- **Method 1 (recommended for CI/CD):** `SUI_ADDRESS` plus `SUI_KEYSTORE`. Both must be present together. `SUI_ADDRESS` is read from a repository variable; `SUI_KEYSTORE` is read from an encrypted secret.
...
### Framework site with build step (Vite, React, Vue, Svelte, and so on)

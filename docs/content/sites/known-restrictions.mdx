...
description: "A comprehensive reference of known restrictions and limitations when developing and deploying Walrus Sites."
keywords: ["walrus sites", "restrictions", "limitations", "development", "secret values", "redirects", "portal restrictions", "service worker", "PWA", "iOS", "epochs", "static", "routing"]
---
...
## Site expiry: epoch-based storage duration

Walrus Sites are not permanently hosted. Storage is paid for in epochs, and a site becomes unavailable once its storage epoch expires unless it is explicitly renewed.

| Network  | Epoch Duration | Maximum Epochs (`max_epochs_ahead`) |
|----------|---------------|--------------------------------------|
| Mainnet  | 14 days       | 53 epochs (~2 years)                 |
| Testnet  | 1 day         | 53 epochs                            |

- **Sites expire:** When the purchased epoch window ends, the site's blobs are no longer guaranteed to be available. Developers must proactively renew storage by running [`site-builder deploy`](/docs/sites/getting-started/using-the-site-builder) again with additional epochs.
- **No automatic renewal:** There is no built-in mechanism to auto-renew storage. Operators are responsible for monitoring and extending epoch coverage.
- **Testnet instability:** The Walrus Testnet may be wiped and redeployed at any time with no notice, which would invalidate all testnet blob IDs. Testnet sites should not be used for production.
- **`wal.app` serves mainnet only:** The public portal at `https://wal.app` only serves sites deployed on Mainnet. To browse a Testnet site, you must run a [local portal](/docs/sites/portals/deploy-locally).
...
The path prefix `/__wal__/*` is reserved by the portal for internal operations such as health checks. **Do not use any resource paths under `/__wal__/` in your site.** Resources served under this path will not be loaded correctly by the portal.
...
- This means iOS wallet users cannot interact with on-chain features (wallet connection, transaction signing) from within a service-worker portal.
...
- **Full resource re-upload on update:** When a site is updated, all resources are deleted and re-uploaded as a [new quilt](/docs/system-overview/quilt), including files that have not changed. There is no differential patching of unchanged assets.
- **Owner-only updates:** The wallet used to run `site-builder deploy` or `site-builder update` must own the Walrus Site object on Sui. Updates from a different wallet will fail.
- **Irreversible destruction:** The `site-builder destroy` command permanently deletes both the Sui blockchain objects and the associated Walrus assets. This action cannot be undone.

## Platform support for `site-builder`

Pre-built binaries for the `site-builder` CLI tool are available for:

- Ubuntu (x86-64 and generic)
- macOS (Apple Silicon and Intel)
- Windows (x86-64)

Other operating systems and architectures are not officially supported via pre-built binaries. Building from source using the Rust toolchain is possible but not documented as a primary path. Most official documentation assumes a Unix-based environment; [Windows users may need to adapt directory paths and shell commands accordingly](/docs/sites/getting-started/installing-the-site-builder#windows).

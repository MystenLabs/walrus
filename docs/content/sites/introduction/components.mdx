...
- Publishing a site does not require managing servers or complex configurations; just provide the
  ...
  [`site-builder` tool](/sites/introduction/technical-overview#the-site-builder), and you are done!
...
explained in a [later section](/sites/portals/deploy-locally), consider for now that there can be many portals (hosted
...
Curious to know how this magic is possible? Read the [technical overview](/sites/introduction/technical-overview)! If you
just want to get started trying Walrus Sites out, check the [tutorial](/docs/sites/index).

...
goal:
  description: Reader understands walrus sites components and can apply the concepts
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  ...
  - >-
    A Walrus Site consists of data stored as blobs on Walrus, a site object on
    Sui that maps resource paths to blob IDs, the site-builder CLI for
    publishing and updating, a ws-resources.?
  - How does walrus sites components work?
answer: >-
  A Walrus Site consists of data stored as blobs on Walrus, a site object on Sui
  that maps resource paths to blob IDs, the site-builder CLI for publishing and
  updating, a ws-resources.json configuration.
---

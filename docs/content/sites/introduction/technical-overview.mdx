...
description: "Conceptual overview of how publishing, resolving, and serving a Walrus Site works end to end."
keywords: ["walrus sites", "technical overview", "site rendering", "portal architecture", "domain isolation", "sui", "walrus"]
---

A Walrus Site has no origin server. Instead, [Sui](docs.sui.io) stores the site's ownership and resource index, and Walrus stores the resource files themselves. A portal bridges these systems and serves the site to the browser over standard HTTP.
...
First, it uploads your site's build directory to Walrus by batching all files into a single storage unit called a [Walrus quilt](/docs/system-overview/quilt). Walrus returns a `QuiltPatchID` for each file.
...
Visitors access a Walrus Site through a portal. A portal is a service that translates a browser request into Sui and Walrus lookups and returns a standard HTTP response. Anyone can run a portal. The Mainnet portal at [https://wal.app](https://wal.app) is operated by Mysten Labs.

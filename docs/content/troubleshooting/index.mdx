...
If you get an error like `the specified Walrus system object does not exist`, make sure your wallet is set up for the correct Sui network (Mainnet or Testnet as you might require) and you use the latest [configuration](/docs/getting-started/index).

## Update to latest Walrus configuration

The Walrus Testnet is wiped periodically and requires updating to the latest binary and configuration. If you get an error like `could not retrieve enough confirmations to certify the blob`, you are probably using an outdated configuration pointing to an inactive Walrus system. In this case, update your configuration file with the latest [configuration](/docs/getting-started/index) and make sure the CLI uses the intended configuration.

...
Ii your configuration file is stored in a non-default location, you can specify a custom path:
...
**Solution:** Retry the read request. Only 1/3 of primary slivers (334 out of 1000) are needed to reconstruct a blob, so the system tolerates many nodes being offline. Try a different aggregator if available, or wait and retry.
...
## Content-Security-Policy errors on Walrus Sites

**Error:**

```
HTTP 503 on route resolution (for example, / returns 503, but /index.html returns 200)
```

**Cause:** The `Content-Security-Policy` header (specifically `default-src 'none'` and `frame-ancestors 'none'`) conflicts with the Walrus portal's request processing pipeline. Direct resource requests bypass route resolution and work, but route redirects (for example, `/` to `/index.html`) fail.

**Solution:** Remove restrictive CSP and security headers (`X-Frame-Options: DENY`, `Permissions-Policy`, `Referrer-Policy`, `X-Content-Type-Options`) from `ws-resources.json`. These headers are appropriate for self-hosted sites but conflict with the Walrus portal's route resolution.
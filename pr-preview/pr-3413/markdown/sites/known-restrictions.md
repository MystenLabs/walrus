...
Walrus Sites can deploy almost any form of traditional static Web2 website built for modern browsers, but developers should keep several restrictions in mind when creating or porting a website to Walrus Sites.
...
- **No server-side rendering (SSR):** Frameworks that require a Node.js (or equivalent) server to render pages per request (such as Next.js in SSR mode, Nuxt.js, or Remix) cannot run their server component on Walrus Sites. Only the pre-built static output of those frameworks, that is, exported HTML/CSS/JS, can be deployed.
...
- Other portals might configure this limit differently. Self-hosted portals can adjust the value as needed.
...
- **Wildcard position is fixed:** Route wildcards (`*`) can only appear at the end of a path pattern. `/path/*` is valid; `/path/*/to` and `*/path/*` are not.
...
**Recommended mitigation:** If using a service-worker portal as your primary access point, redirect iOS wallet users to a server-side portal (for example, `https://wal.app`). This allows the wallet interaction to function correctly through the `<site_name>.wal.app` server-side endpoint.

### Progressive web apps are not supported

Service-worker portals are fundamentally incompatible with Progressive Web Apps (PWAs) because of two structural conflicts:
...
The server-side portal does not share these limitations. Walrus Sites must be compatible with both portal types, so they must target the more restrictive service-worker feature set. For this reason, PWAs cannot currently be deployed to Walrus Sites while maintaining full portal compatibility.

## Domain and naming constraints

- **SuiNS dependency for human-readable URLs:** Without a [SuiNS name](/docs/sites/custom-domains/setting-a-suins-name), a site is only accessible through a Base36-encoded Sui object ID subdomain (for example, `58gr4pinoayuijgdixud23441t55jd94ugep68fsm72b8mwmq2.wal.app`). This is because subdomains are case-insensitive (ruling out Base64/Base58) and limited to 63 characters (ruling out hex, which requires 64).

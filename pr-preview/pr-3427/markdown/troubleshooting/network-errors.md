...
**Solution:** Acquire WAL through the official path documented in [Getting Started](/docs/getting-started) or [Testnet WAL faucet](/docs/system-overview/available-networks#testnet-wal-faucet), not third-party faucets. On Testnet, use the WAL exchange (`walrus get-wal`); on Mainnet, swap SUI for WAL through a supported exchange. WAL from an unsupported package cannot be used for storage payments.
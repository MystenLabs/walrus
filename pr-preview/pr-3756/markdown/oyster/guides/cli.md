...
> **Warning**
>
> The URL must include the `/api/v1` path. The CLI appends
...
[S3 object-tagging](../s3-api/objects.mdx#object-tagging) operations. Limits:
...
not through the CLI. See the [Admin API docs](../json-api/admin.mdx) for
...
([Set Webhook URL](../json-api/admin.mdx#set-webhook-url)) using the active
...
See [Webhooks](webhooks.mdx) for the delivery signature format.

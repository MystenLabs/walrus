...
2. Open **Integrations**, then click **Create API Key** in the top-right corner. Give the key a name and choose its type:
   - **Management API key** mints further API keys programmatically through the CLI. It cannot upload, download, or manage assets; use an API key for that.
   - **API key** cannot mint further API keys. The choice is not permanent, so you can create a Management API key at any time.
3. If you chose **Management API key**, the reveal screen shows two secrets. Copy both, because Console shows them once and cannot recover them afterward:
   ...
   Store both like cloud secret access keys. Paste them into your `.env` as your bearer token and `CONSOLE_SERVICE_PRIVKEY`, or into Postman.

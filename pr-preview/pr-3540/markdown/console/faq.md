...
Everything programmatic. The API, the MCP server, and uploads from your own code all authenticate with it. You mint keys under **Integrations** in Console, choose a `read_write` or `read_only` role for each, and revoke a key whenever you want. Pick a plain **API key** for the API, the MCP server, and your own code; a **Management API key** mints further keys programmatically but cannot upload, download, or manage assets. Console shows the full key once, at creation, so store it like a cloud secret access key. See [API keys and roles](./overview#api-keys-and-roles).

...
Console keeps a perpetual free tier so new developers are not paywalled. Console plans a free storage cap, tentatively 5 GB, pending analysis of real Mainnet usage. Usage-based billing for reads and egress, along with a paid top-up path, follows after GA. Console manages WAL token handling on your behalf.

...
**Cause:** The `site-builder` cannot find its configuration file. By default, it searches the current directory, `$XDG_HOME/walrus/sites-config.yaml`, and `$HOME/.config/walrus/sites-config.yaml`, in that order.
...
Replace `mainnet` with `testnet` if you are deploying to Testnet. If you store the file in a non-default location, pass its path explicitly:
...
**Cause:** Walrus stores blobs for a fixed number of epochs. When the storage period expires, the blobs become unavailable and the site stops loading. On Mainnet, each epoch is approximately 14 days. On Testnet, each epoch is approximately 1 day.

**Solution:** Redeploy the site with additional epochs:

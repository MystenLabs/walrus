...
If you haven't installed certbot from the [storage node setup
...
##### Step 2: Add a cache path directive to the `http` block in `/etc/nginx/nginx.conf`:
...
##### Step 3: Create a site configuration at `/etc/nginx/sites-available/<YOUR_HOSTNAME>`:
...
##### Step 4: Validate the configuration: `sudo nginx -t`
...
[storage node TLS setup](/docs/operator-guide/storage-node-setup#tls-setup) to learn more about how certbot is

This page covers the ongoing operation of your Walrus storage node after
[initial setup](/docs/operator-guide/storage-node-setup).

## Important data to back up

Back up the `/opt/walrus/config` directory. For database backups, see the
[Backup and Restore](/docs/operator-guide/backup-restore-guide) guide.
...
Other metrics like `walrus_storage_confirmations_issued_total` should also increase regularly, but
they depend on user activity.
...
Operators are generally expected to upgrade within 24 hours of a new release. In emergency
situations, immediate action is appreciated. Subscribe to the
[Walrus release calendar](https://calendar.google.com/calendar/u/0/embed?src=c_97763fcda7894da7ddcd68595a797397b9b4294b69603a52e30d4fa0c3fee2bb@group.calendar.google.com)
to stay informed about upcoming releases.

:::

## Database repair

If the node database becomes corrupted (for example, after an unclean shutdown), you can attempt a
repair:
...
If the repair is unsuccessful, restore from a backup. See the
[Backup and Restore](/docs/operator-guide/backup-restore-guide) guide.

## Updating on-chain parameters

To modify node parameters (capacity, voting parameters, metadata, and others), edit the
`/opt/walrus/config/walrus-node.yaml` file. The node automatically picks up changes and updates
on-chain information. See the [FAQ on TLS](/docs/operator-guide/storage-node-faq#tls) for details
on how automatic configuration updates work.

:::tip

Avoid changing the node name, keys, and network address unless necessary, as this causes some
...
Several community members have created tools for monitoring Walrus services, listed on

...
[S3 Setup](setup.mdx) for configuration.

> **Authenticated reads:** Unlike the [JSON API](../json-api/blobs.mdx), where
...
Tags set this way share the same store as the [JSON API](../json-api/blobs.mdx#blob-tags)
...
[JSON API tag endpoints](../json-api/blobs.mdx#blob-tags); a tag set through S3
is visible through the JSON API and vice versa. The same
[tag rules](../json-api/blobs.mdx#tag-rules) apply (max 10 tags; key ≤ 128 B;

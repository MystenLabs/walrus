...
See [Limitations](limitations.mdx) for what's not yet supported compared to
full AWS S3.

## Authentication

S3 requests are authenticated using **S3 access keys** (created through the
[JSON API](../json-api/access-keys.mdx)). The AWS SDK and CLI handle SigV4
...
Head to [S3 Setup](setup.mdx) to configure your AWS CLI or SDK.
The Walrus SDK interacts with the storage network through storage nodes, which act as both publishers (for writes) and aggregators (for reads).

When uploading without a relay, he SDK queries the system state to get the current committee of storage nodes. Each node is responsible for specific shards based on the committee structure. The SDK uploads slivers to multiple nodes in parallel.

        ...
        loop {
            if start.elapsed() > timeout {
                ...
                return Err(anyhow::anyhow!(
                    "Timeout waiting for primary index {} after {:?}. \n\
                    Expected: {:?}\n\
                    Found: {:?}\n\
                    Bucket ID: {}\n\
                    Identifier: {}",
            ...
            match (index_target.as_ref(), expected_target) {
                (Some(IndexTarget::Blob(found)), IndexTarget::Blob(expected)) => {
                    if found.blob_id == expected.blob_id
                        && found.quilt_status == expected.quilt_status
                    {
                        // Found matching entry, move to next one
                        break;
                    }
                    // Mismatch, retry
                    tracing::debug!(
                        "Retrying primary index {}: blob_id or status mismatch",
                        primary_index
                    );
                }
                (Some(IndexTarget::QuiltPatchId(found)), IndexTarget::QuiltPatchId(expected)) => {
                    ...
                    // Mismatch, retry
                    tracing::debug!(
                        "Retrying primary index {}: patch ID mismatch",
                        primary_index
                    );
                }
                (None, _) => {
                    // Not found yet, retry
                    tracing::debug!("Retrying primary index {}: not found yet", primary_index);
                }
                _ => {
                    // Type mismatch, retry
                    tracing::debug!("Retrying primary index {}: type mismatch", primary_index);
                }
            }
    ...
    for (patch_blob_id, expected_patch_id) in patch_blob_mapping {
        loop {
            if start.elapsed() > timeout {
                ...
                return Err(anyhow::anyhow!(
                    "Timeout waiting for quilt patch blob_id {:?} after {:?}. \n\
                    Expected patch_id: {:?}\n\
                    Found patch_ids: {:?}\n\
            ...
            if !found_patch_ids.is_empty() {
                ...
                if found == expected_patch_id {
                    // Found matching entry, move to next one
                    break;
                }
                // Mismatch, retry
                tracing::debug!(
                    "Retrying quilt patch {:?}: patch ID mismatch",
                    patch_blob_id
                );
            } else {
                // Not found yet, retry
                tracing::debug!("Retrying quilt patch {:?}: not found yet", patch_blob_id);
            }
...
async fn test_walrus_indexer_basic() -> Result<()> {
    ...
    // === PHASE 3: Verify indexed data ===
    // Check that all blobs were properly indexed with a timeout.

    // Verify primary index mappings for blobs from both buckets.
    verify_primary_index_mappings(
        &indexer,
        bucket_id_1,
        &blob_mapping_1,
    ...
    // === PHASE 4: Verify quilt patches ===
    // Check that all quilt patches were properly indexed.
    // Quilt processing happens asynchronously, so we use a longer timeout.

    // Verify quilt primary index mappings and patch blob mappings.
    verify_primary_index_mappings(
        &indexer,
        bucket_id_1,
        &quilt_primary_mapping,
        Duration::from_secs(60),
    )
    .await?;
    verify_quilt_patch_mappings(
        &indexer,
        &quilt_patch_blob_mapping,
...
async fn test_walrus_indexer_catchup() -> Result<()> {
    ...
    // Verify all primary index mappings for blobs with timeout.
    verify_primary_index_mappings(
        &indexer,
        bucket_id,
        &initial_blob_mapping,
        Duration::from_secs(60),
    )
    .await?;
    verify_primary_index_mappings(
    ...
    verify_primary_index_mappings(
        &indexer,
        bucket_id,
        &additional_quilt_mapping,
        Duration::from_secs(60),
    )
    .await?;
    verify_quilt_patch_mappings(
        &indexer,
        &initial_quilt_patch_mapping,
        Duration::from_secs(120),
    )
    .await?;
    verify_quilt_patch_mappings(
        &indexer,
        &additional_quilt_patch_mapping,
        Duration::from_secs(120),
    )
    .await?;

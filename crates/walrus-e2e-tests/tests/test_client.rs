...
<<<<<<< HEAD
    let (_sui_cluster_handle, _cluster, client, _, _) = test_cluster_builder.build().await?;
=======
    let (_sui_cluster_handle, walrus_cluster, client, _) = test_cluster_builder.build().await?;
>>>>>>> 619066f53 (add tests)
    let client = client.as_ref();
    ...
    if with_node_failure {
        ...
        tracing::info!("ZZZZZ stopping node {node_to_stop}");
        // Fail a node.
...
// Tests recovering primary slivers and reconstructing the blob.
// This test uploads a blob, recovers all primary slivers, and verifies the reconstructed blob
// matches the original.
async fn test_recover_primary_slivers_and_reconstruct<E: EncodingAxis>(
    client: &WalrusNodeClient<SuiContractClient>,
) -> TestResult {
...
/// Tests the recover_slivers functionality.
/// This test uploads a blob, reads all primary slivers, uses recover_slivers to reconstruct them,
/// and verifies the reconstructed slivers match the original data.
#[ignore = "ignore E2E tests by default"]
#[walrus_simtest]
async fn test_recover_slivers_client() -> TestResult {
    walrus_test_utils::init_tracing();
    ...
    for _ in 0..10 {
        test_recover_primary_slivers_and_reconstruct::<Primary>(client).await?;
        test_recover_primary_slivers_and_reconstruct::<Secondary>(client).await?;
    }

...
pub fn compile_package(package_path: PathBuf) -> (PackageDependencies, Vec<Vec<u8>>) {
    ...
    let resolution_graph = build_config.resolution_graph(&package_path).expect("Resolution failed");
...
pub async fn publish_package(
    ...
    let publish_tx = sui
        .transaction_builder()
        .publish(sender, compiled_modules, dep_ids, None, 10000000000).await?;
    ...
    ensure!(transaction_response.status_ok() == Some(true), "Status not ok");

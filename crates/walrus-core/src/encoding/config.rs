...
impl EncodingConfig {
    pub(crate) fn new(
        ...
        let _ = SliverIndex::try_from(n_shards)
            .expect("the number of shards exceeds the sliver index limit");
    ...
    pub fn sliver_index_from_pair_index<T: EncodingAxis>(
        ...
        if T::IS_PRIMARY {
            ...
            SliverIndex(idx.try_into().expect("shard index out of bounds"))
        }
    }

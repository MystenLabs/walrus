...
/// Configuration for gRPC migration in the retriable client.
///
/// Fields indicate whether to use the new gRPC client for specific operations.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
struct GrpcMigrationConfig {
    /// Whether to use the new gRPC client for fetching objects.
    pub get_object: bool,
    /// Whether to use the new gRPC client for fetching the type origin map.
    pub type_origin_map: bool,
}

impl Default for GrpcMigrationConfig {
    /// Enable all gRPC migration options in tests.
    #[cfg(test)]
    fn default() -> Self {
        Self {
            get_object: true,
            type_origin_map: true,
        }
    }
    /// Selectively enable gRPC migration options outside of tests.
    #[cfg(not(test))]
    fn default() -> Self {
        Self {
            get_object: std::env::var("WALRUS_GRPC_GET_OBJECT").is_ok_and(|v| v != "0"),
            type_origin_map: std::env::var("WALRUS_GRPC_TYPE_ORIGIN_MAP").is_ok_and(|v| v != "0"),
        }
    }
}
...
pub struct RetriableSuiClient {
    ...
    grpc_migration_config: GrpcMigrationConfig,
}
...
impl RetriableSuiClient {
    ...
    pub fn new(
        ...
        Ok(RetriableSuiClient {
            ...
            backoff_config,
            grpc_migration_config: GrpcMigrationConfig::default(),
            metrics: None,
        })
    }
    ...
    async fn get_object_by_grpc(
        ...
        debug_assert!(self.grpc_migration_config.get_object);
        async fn make_request(
            client: Arc<DualClient>,
            object_id: ObjectID,
        ) -> SuiClientResult<sui_types::object::Object> {
    ...
    {
        if self.grpc_migration_config.get_object {
    ...
    pub(crate) async fn type_origin_map_for_package(
        ...
        if self.grpc_migration_config.type_origin_map {
            async fn make_request(
                client: Arc<DualClient>,
                package_id: ObjectID,
            ) -> SuiClientResult<TypeOriginMap> {

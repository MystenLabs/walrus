...
impl FallibleRpcClient {
    ...
    pub fn new(rpc_url: String) -> anyhow::Result<Self> {
        let client = rpc_client::create_client_with_env_auth(&rpc_url)?;
        Ok(Self {
            rpc_url,
        ...
        Fut: Future<Output = Result<T, tonic::Status>>,
    {
        ...
        let Ok(inner_result) = result else {
                ...
                && self.num_failures.load(Ordering::Relaxed) > Self::MAX_FAILURES
            {
                ...
                *client = rpc_client::create_client_with_env_auth(&self.rpc_url).map_err(|e| {
                    tracing::error!("failed to recreate rpc client: {}", e);

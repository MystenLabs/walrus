...
impl WalrusPtbBuilder {
    ...
    /// [`Argument`] representing the new `StoragePool` object.
    pub async fn create_storage_pool(
        ...
        let price = self
            .storage_price_for_encoded_length(storage_amount, epochs_ahead, false)
            .await?;
        self.fill_wal_balance(price).await?;

        let args = vec![
            self.system_arg(SharedObjectMutability::Mutable)?,
            self.pt_builder.pure(storage_amount)?,
            self.pt_builder.pure(epochs_ahead)?,
            self.wal_coin_arg()?,
        ];
    ...
    /// Adds a call to `register_pool_blob` to the `pt_builder`.
    pub async fn register_pool_blob(
        ...
        self.walrus_move_call(contracts::system::register_pool_blob, args)?;
        self.reduce_wal_balance(price)?;
        Ok(())
    }

    /// Adds a call to `certify_pool_blob` to the `pt_builder`.
    pub async fn certify_pool_blob(
        ...
        self.walrus_move_call(contracts::system::certify_pool_blob, args)?;
        Ok(())
    }

    /// Adds a call to `delete_pool_blob` to the `pt_builder`.
    pub async fn delete_pool_blob(
        ...
        self.walrus_move_call(contracts::system::delete_pool_blob, args)?;
        Ok(())
    }
    ...
    pub async fn extend_storage_pool(
        ...
        let price = self
            .storage_price_for_encoded_length(storage_size, extended_epochs, false)
            .await?;
        self.fill_wal_balance(price).await?;

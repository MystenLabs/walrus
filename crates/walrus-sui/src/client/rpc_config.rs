...
impl RpcEndpointAuth {
    ...
    pub fn has_auth(&self) -> bool {
        (self.username.is_some() && self.password.is_some()) || self.bearer_token.is_some()
    }
        ...
        for (key, value) in std::env::vars() {
            if let Some(rest) = key.strip_prefix("WALRUS_RPC_AUTH_") {
                ...
                for suffix in SUFFIXES {
                    if let Some(url_encoded) = rest.strip_suffix(suffix) {
                        matched = true;
                        match urlencoding::decode(url_encoded) {
                            Ok(decoded_url) => {
                                let url = decoded_url.into_owned();
                                let endpoint_auth = config.endpoints.entry(url).or_default();
                                match suffix {
                                    "_USERNAME" => endpoint_auth.username = Some(value),
                                    "_PASSWORD" => endpoint_auth.password = Some(value),
                                    "_BEARER_TOKEN" => endpoint_auth.bearer_token = Some(value),
                                    _ => unreachable!(),
                                }
                            }
                            Err(e) => {
                ...
                if !matched {
                    tracing::warn!(
                        "Ignoring environment variable with unrecognized auth suffix: {}",
                        key
                    );
                }
            }

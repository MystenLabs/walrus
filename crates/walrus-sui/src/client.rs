...
impl SuiContractClient {
    ...
    // === Storage Pool (bucket) operations ===
    ...
    /// Registers a blob against a storage pool.
    ///
    /// Returns the object ID of the created `PoolBlob` object.
    pub async fn register_pool_blob(
        &self,
        ...
        self.retry_on_wrong_version(|| async {
            self.inner
                .lock()
                .await
                .register_pool_blob(storage_pool_id, blob_metadata.clone(), persistence)
                .await
        })
        .await
    }
    ...
    pub async fn certify_pool_blob(
        &self,
        storage_pool_id: ObjectID,
        blob_id: BlobId,
        ...
        self.retry_on_wrong_version(|| async {
            self.inner
                .lock()
                .await
                .certify_pool_blob(storage_pool_id, blob_id, certificate)
                .await
        })
        .await
    }
    ...
    pub async fn delete_pool_blob(
        ...
        self.retry_on_wrong_version(|| async {
            self.inner
                .lock()
                .await
                .delete_pool_blob(storage_pool_id, blob_id)
                .await
        })
        .await
    }
    ...
    pub async fn extend_storage_pool(
        ...
        self.retry_on_wrong_version(|| async {
            self.inner
                .lock()
                .await
                .extend_storage_pool(storage_pool_id, extended_epochs, storage_size)
                .await
        })
        .await
    }
...
impl SuiContractClientInner {
    ...
    pub async fn create_storage_pool(
        ...
        // Extract the created object ID from the response.
        let created_objects = response
            .effects
            .as_ref()
            .ok_or_else(|| anyhow!("no transaction effects"))?
            .created()
            .iter()
            .map(|o| o.object_id())
            .collect::<Vec<_>>();
        ensure!(!created_objects.is_empty(), "no objects created");
        Ok(created_objects[0])
    }
    ...
    pub async fn register_pool_blob(
        ...
        let mut pt_builder = self.transaction_builder();
        pt_builder
            .register_pool_blob(storage_pool_id.into(), blob_metadata, persistence)
            .await?;
        let transaction = pt_builder.build_transaction_data(self.gas_budget).await?;
        let response = self
            .sign_and_send_transaction(transaction, "register_pool_blob")
            .await?;

        let blob_obj_ids = get_created_sui_object_ids_by_type(
            &response,
            &contracts::storage_pool::PoolBlob
                .to_move_struct_tag_with_type_map(&self.read_client.type_origin_map(), &[])?,
        )?;
        ensure!(
            blob_obj_ids.len() == 1,
            "expected 1 PoolBlob object, got {}",
            blob_obj_ids.len()
        );
        Ok(blob_obj_ids[0])
    }
    ...
    pub async fn certify_pool_blob(
        ...
        let mut pt_builder = self.transaction_builder();
        pt_builder
            .certify_pool_blob(storage_pool_id.into(), blob_id, certificate)
            .await?;
        let transaction = pt_builder.build_transaction_data(self.gas_budget).await?;
        self.sign_and_send_transaction(transaction, "certify_pool_blob")
            .await?;
        Ok(())
    }
    ...
    pub async fn delete_pool_blob(
        ...
        let mut pt_builder = self.transaction_builder();
        pt_builder
            .delete_pool_blob(storage_pool_id.into(), blob_id)
            .await?;
        let transaction = pt_builder.build_transaction_data(self.gas_budget).await?;
        self.sign_and_send_transaction(transaction, "delete_pool_blob")
            .await?;
        Ok(())
    }
    ...
    pub async fn extend_storage_pool(
        ...
        let mut pt_builder = self.transaction_builder();
        pt_builder
            .extend_storage_pool(storage_pool_id.into(), extended_epochs, storage_size)
            .await?;
        let transaction = pt_builder.build_transaction_data(self.gas_budget).await?;

...
//! End-to-end simtests for storage pool (bucket) operations.
...
mod tests {
    ...
    /// Register a single pooled blob, upload its slivers, and certify it.
    async fn store_blob(
        ...
        let pooled_blobs = sui_client
            .register_pooled_blobs(bucket_id, vec![blob_metadata], persistence)
            .await?;
        let pooled_blob: PooledBlob = pooled_blobs
            .into_iter()
            .next()
            .ok_or_else(|| anyhow::anyhow!("expected one pooled blob from registration"))?;
    ...
    /// Listing blobs via `list_pooled_blob_ids`, verifying blob_count and ids.
    #[ignore = "ignore integration simtests by default"]
    #[walrus_simtest]
    async fn test_storage_pool_list_blobs() {
        ...
        assert!(sui.list_pooled_blob_ids(bucket).await.unwrap().is_empty());
        ...
        // Verify listing by blob id.
        let listed: HashSet<BlobId> = sui
            .list_pooled_blob_ids(bucket)
            .await
            .unwrap()
            .into_iter()
            .collect();
        ...
        assert_eq!(sui.storage_pool_status(bucket).await.unwrap().blob_count, 3);

        // Distinct registrations create distinct object IDs.
        assert_ne!(obj1, obj3);
        ...
        assert!(listed.contains(&id3));
        assert!(!listed.contains(&id2));
        assert_eq!(sui.storage_pool_status(bucket).await.unwrap().blob_count, 2);
    ...
    async fn test_storage_pool_increase_capacity() {
        ...
        let pool = sui.storage_pool_status(bucket).await.unwrap();
        ...
        assert!(
            sui.register_pooled_blobs(bucket, vec![bm], BlobPersistence::Deletable)
                .await
                .is_err()
        );
        ...
        sui.increase_storage_pool_capacity(bucket, 10 * 1024 * 1024)
            .await
            .unwrap();
        let pool2 = sui.storage_pool_status(bucket).await.unwrap();
    ...
    async fn test_storage_pool_same_blob_two_pools_delete_lifecycle() {
        ...
        let pool1 = sui.storage_pool_status(p1).await.unwrap();
        let pool2 = sui.storage_pool_status(p2).await.unwrap();
        ...
        let pool1_after_delete = sui.storage_pool_status(p1).await.unwrap();
        let pool2_after_delete = sui.storage_pool_status(p2).await.unwrap();
        ...
        assert_eq!(sui.storage_pool_status(p2).await.unwrap().blob_count, 0);
    ...
    async fn test_storage_pool_permanent_blob() {
        ...
        assert_eq!(sui.storage_pool_status(bucket).await.unwrap().blob_count, 1);
    ...
    async fn test_storage_pool_state_accounting() {
        ...
        let pool_before = sui.storage_pool_status(bucket).await.unwrap();
        ...
        let pool_after_store = sui.storage_pool_status(bucket).await.unwrap();
        assert_eq!(
        ...
        let listed = sui.list_pooled_blob_ids(bucket).await.unwrap();
        assert_eq!(listed, vec![blob_id]);
        // We know the object id from registration; it must be distinct from the blob id table key.
        assert_ne!(blob_obj_id, bucket);

        sui.delete_pooled_blob(bucket, blob_id).await.unwrap();
        wait_until_blob_unreadable(&ctx.client, &blob_id).await;

        let pool_after_delete = sui.storage_pool_status(bucket).await.unwrap();
        assert_eq!(
        ...
        sui.increase_storage_pool_capacity(bucket, extra_capacity)
            .await
            .unwrap();
        let pool_after_capacity = sui.storage_pool_status(bucket).await.unwrap();
        assert_eq!(
        ...
        sui.extend_storage_pool(bucket, 2).await.unwrap();
        let pool_after_extend = sui.storage_pool_status(bucket).await.unwrap();
        assert_eq!(
    ...
    async fn test_storage_pool_delete_and_reuse_capacity() {
        ...
        assert_eq!(sui.storage_pool_status(bucket).await.unwrap().blob_count, 0);
    ...
    async fn test_storage_pool_expired_pool_rejects_mutations() {
        ...
        assert!(
            sui.register_pooled_blobs(bucket, vec![blob_metadata], BlobPersistence::Deletable)
                .await
                .is_err()
        );

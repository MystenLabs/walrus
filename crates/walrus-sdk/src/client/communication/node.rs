...
pub struct NodeCommunication<W = ()> {
    ...
    pub auto_tune_handle: Option<AutoTuneHandle>,
    pub throughput_stats: Arc<Mutex<NodeThroughputStats>>,
...
impl NodeWriteCommunication {
        ...
        match self.store_sliver(blob_id, sliver, pair_index).await {
            Ok(()) => {
                ...
                if let Some(handle) = &self.auto_tune_handle {
                    handle.record_success(sliver.len(), A::sliver_type(), completed_at);
                }
                self.record_throughput_success(sliver.len(), completed_at.duration_since(start));
                Ok(())
            }
            Err(error) => Err(error),
        }
    ...
    fn record_throughput_success(&self, bytes: usize, elapsed: Duration) {

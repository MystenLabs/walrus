...
use tokio::{select, sync::Mutex, task::JoinHandle, time::sleep};
...
const STARTUP_CATCHUP_TIMEOUT_SECS: u64 = 300 * 5;
...
impl EventProcessor {
    ...
    pub async fn new(
        ...
        let coordination_state = Arc::new(CatchupCoordinationState::new());
        ...
        let catchup_manager = EventBlobCatchupManager::new(
            ...
            catchup::CatchupRuntimeConfig {
                ...
                processing_timeout: Duration::from_secs(STARTUP_CATCHUP_TIMEOUT_SECS),
            },
        );
    ...
    pub async fn start(&self, cancellation_token: CancellationToken) -> Result<(), anyhow::Error> {
        tracing::info!(
            runtime_catchup_enabled = self.config.enable_runtime_catchup,
            "starting event processor with runtime catchup support"
        );
        ...
        let catchup_monitoring_task = if self.config.enable_runtime_catchup {
            Some(self.start_runtime_catchup_monitoring(
                cancellation_token.clone(),
                coordination_state.clone(),
            ))
        } else {
        ...
        if let Some(catchup_task) = catchup_monitoring_task {
            select! {
    ...
    fn start_tailing_task(
        ...
        tokio::spawn(async move { processor.start_tailing_checkpoints(cancel_token).await })
    }
    ...
    async fn start_tailing_checkpoints_with_restart_support(
        ...
        let mut coordination_rx = coordination_state.get_coordination_receiver().await;
        ...
        loop {
            tokio::select! {
                ...
                join_res = async {
                    tailing_task
                    .take()
                    .expect("tailing_task present").await }, if tailing_task.is_some() => {
                    match join_res {
                        Ok(Ok(_)) => tracing::info!("tailing task exited"),
                        Ok(Err(e)) => tracing::error!(?e, "tailing task error"),
                        Err(e) => tracing::error!(?e, "tailing task panicked"),
                    }
                    coordination_state.notify_tailing_stopped();
                    continue;
                }
                    ...
                    match catchup_manager.get_current_lag().await {
                        Ok(lag) => {
                            ...
                            if lag > self.config.runtime_catchup_lag_threshold {
                                ...
                                catchup_manager.perform_catchup().await?;
                            } else {
                                tracing::debug!(
                                    lag = lag,
                                    threshold = self.config.runtime_catchup_lag_threshold,

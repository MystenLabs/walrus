...
const STARTUP_CATCHUP_TIMEOUT_SECS: u64 = 25 * 60;
...
impl EventProcessor {
    ...
    pub async fn new(
        ...
        let (coordination_state, _coordination_rx) = CatchupCoordinationState::new();
    ...
    pub async fn start(&self, cancellation_token: CancellationToken) -> Result<(), anyhow::Error> {
        if self.config.enable_runtime_catchup {
            tracing::info!(
                runtime_catchup_enabled = true,
                "starting event processor with runtime catchup enabled"
            );
        } else {
            tracing::info!(
                runtime_catchup_enabled = false,
                "starting event processor with runtime catchup disabled"
            );
        }
                ...
                msg = coordination_rx.recv() => {
                    match msg {
                        Some(CoordinationMessage::StopCheckpointTailing) => {
                            ...
                            match tailing_task.take().expect("tailing task not running").await {

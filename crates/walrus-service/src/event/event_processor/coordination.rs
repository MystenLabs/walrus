...
pub struct CatchupCoordinationState {
    ...
    /// Channel for receiving coordination messages in checkpoint tailing task
    pub coordination_rx: Arc<Mutex<mpsc::UnboundedReceiver<CoordinationMessage>>>,
    /// Whether catchup is currently active (downloading or processing)
    pub catchup_active: Arc<std::sync::atomic::AtomicBool>,
    /// Timestamp when catchup was last started (for cooldown management)
...
impl CatchupCoordinationState {
    /// Creates a new coordination state instance.
    pub fn new() -> Self {
        let (coordination_tx, coordination_rx) = mpsc::unbounded_channel();

        Self {
            coordination_tx,
            coordination_rx: Arc::new(Mutex::new(coordination_rx)),
    ...
    /// Get a receiver for coordination messages (for checkpoint tailing task)
    pub async fn get_coordination_receiver(&self) -> mpsc::UnboundedReceiver<CoordinationMessage> {
        // Take the receiver out of the Arc<Mutex<>> - this should only be called once
        let mut rx_guard = self.coordination_rx.lock().await;
        let (_new_tx, new_rx) = mpsc::unbounded_channel();

        // Replace the old receiver with a new one (though it won't be used)
...
mod tests {
    ...
    async fn test_coordination_message_passing() {
        let state = CatchupCoordinationState::new();
        let mut rx = state.get_coordination_receiver().await;

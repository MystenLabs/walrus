...
pub enum StoreMetadataError {
    #[error(transparent)] InvalidMetadata(#[from] VerificationError),
    #[error(transparent)] Internal(#[from] anyhow::Error),
}

#[derive(Debug, thiserror::Error)]
pub enum RetrieveSliverError {
    #[error("Invalid shard {0:?}")] InvalidShard(ShardIndex),
    #[error(transparent)] Internal(#[from] anyhow::Error),
}
...
pub enum StoreSliverError {
    #[error("Missing metadata for {0:?}")] MissingMetadata(BlobId),
    #[error("Invalid sliver pair id {0} for {1:?}")] InvalidSliverPairId(u16, BlobId),
    #[error("Invalid sliver id {0} for {1:?}")] InvalidSliver(u16, BlobId),
    #[error("Invalid sliver size {0} for {1:?}")] IncorrectSize(usize, BlobId),
    #[error("Invalid shard type {0:?} for {1:?}")] InvalidSliverType(SliverType, BlobId),
    #[error("Invalid shard {0:?}")] InvalidShard(ShardIndex),
    #[error(transparent)] MalformedSliver(#[from] RecoveryError),
    #[error(transparent)] Internal(#[from] anyhow::Error),
}
...
impl ServiceState for StorageNode {
    fn retrieve_metadata(
        ...
        let verified_metadata_with_id = self.storage
    ...
    fn store_sliver(
        ...
        let shard = shard_index_for_pair(sliver_pair_idx, self.n_shards.get(), blob_id);
        let shard_storage = self.storage
            .shard_storage(shard)
        ...
        shard_storage.put_sliver(blob_id, sliver).context("unable to store sliver")?;
    ...
    async fn compute_storage_confirmation(
        ...
        if self.storage.is_stored_at_all_shards(blob_id)? {
            ...
            sign_confirmation(confirmation, self.signer.clone()).await.map(|signed|
                Some(StorageConfirmation::Signed(signed))
            )
        } else {
            Ok(None)
        }
        ...
        where U: HashFunction<DIGEST_LEN>
    {
        let optional_sliver = self
            ...
            .map_err(|_| {
                RetrieveSymbolError::InvalidShard(
                    shard_index_for_pair(sliver_pair_idx, self.n_shards.get(), blob_id)
                )
            })?;
...
async fn sign_confirmation(
    ...
    let signed = tokio::task
        ::spawn_blocking(move || {
            let encoded_confirmation = bcs
                ::to_bytes(&confirmation)
    ...
    mod get_storage_confirmation {
        ...
        async fn returns_none_if_no_shards_store_pairs() -> TestResult {
            ...
            let confirmation = storage_node
                .as_ref()
                .compute_storage_confirmation(&BLOB_ID).await
        ...
        async fn returns_confirmation_over_nodes_storing_the_pair() -> TestResult {
            ...
            let confirmation: Confirmation = bcs
                ::from_bytes(&signed.confirmation)

...
mod epoch_change;
pub use epoch_change::BeginCommitteeChangeAction;
...
impl StorageNode {
    async fn new(
        ...
        let shard_sync_handler = Arc::new(ShardSyncHandler::new(
            inner.clone(),
            config.shard_sync_config.clone(),
        ));
        ...
        let start_epoch_change_finisher = Arc::new(StartEpochChangeFinisher::new(inner.clone()));

        let node_recovery_handler = Arc::new(NodeRecoveryHandler::new(
            inner.clone(),
            blob_sync_handler.clone(),
            shard_sync_handler.as_ref().clone(),
    ...
    pub(crate) fn is_stored_at_active_shards_snapshot(
        ...
        for shard_storage in active_shard_storages {
            if !shard_storage.is_sliver_pair_stored(blob_id)? {
                let shard = shard_storage.id();
                tracing::error!(%blob_id, %shard, "DIAG: blob sliver pair missing at shard");
                return Ok(false);
            }
        }
        Ok(true)

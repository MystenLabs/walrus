...
impl StorageNode {
    async fn new(
        ...
        let last_certified_event_blob_metadata =
            event_blob_downloader.as_ref().map(|downloader| async {
                downloader
        ...
        let last_certified_event_blob = match last_certified_event_blob_metadata {
            Some(metadata_future) => match metadata_future.await {
                Some(metadata) => Some(LastCertifiedEventBlob::EventBlobWithMetadata(metadata)),
                None => contract_service

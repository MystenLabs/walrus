...
fn available_disk_space(path: &Path) -> std::io::Result<u64> {
    ...
    // SAFETY: `stat` is a valid, zero-initialized `statvfs` and `c_path` is a valid,
    ...
    // `f_bavail` counts the blocks (of `f_frsize` bytes) available to unprivileged users. Their
    // integer widths are platform-dependent, so the conversions are needed for portability even
    // where they are identity conversions.
...
impl Storage {
    ...
    /// Returns the number of bytes of free disk space available on the filesystem backing the
    /// database.

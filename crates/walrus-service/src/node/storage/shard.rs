...
impl ShardStorage {
    ...
    pub(crate) fn create_or_reopen(
        ...
        db_options: &DatabaseTableOptionsFactory,
    ...
    fn create_or_reopen_inner(
        ...
        let shard_status = reopen_cf!(
            (
                ...
                shard_status_column_family_options(db_options),
            ),
            database,
            rw_options
        );
        let shard_sync_progress = reopen_cf!(
            (
                ...
                shard_sync_progress_column_family_options(db_options),
            ),
            database,
            rw_options
        );
        let pending_recover_slivers = reopen_cf!(
            (
                ...
                pending_recover_slivers_column_family_options(db_options),
            ),
            database,
            rw_options
        );
        ...
        let primary_slivers = reopen_cf!(
            (
                ...
                primary_slivers_column_family_options(db_options)
            ),
            database,
            rw_options
        );
        let secondary_slivers = reopen_cf!(
            (
                ...
                secondary_slivers_column_family_options(db_options)
            ),
            database,
            rw_options
        );
...
pub fn primary_slivers_column_family_options(db_options: &DatabaseTableOptionsFactory) -> Options {
    db_options.shard().to_options()
}

/// Returns the name and options for the column families for a shard's secondary
/// sliver with the specified index.
pub fn secondary_slivers_column_family_options(
    db_options: &DatabaseTableOptionsFactory,
) -> Options {
    db_options.shard().to_options()
}
...
pub fn shard_status_column_family_options(db_options: &DatabaseTableOptionsFactory) -> Options {
    db_options.shard_status().to_options()
}

/// Returns the name and options for the column families for a shard's sync progress
/// with the specified index.
pub fn shard_sync_progress_column_family_options(
    db_options: &DatabaseTableOptionsFactory,
) -> Options {
    db_options.shard_sync_progress().to_options()
}

/// Returns the name and options for the column families for a shard's pending recover slivers
/// with the specified index.
pub fn pending_recover_slivers_column_family_options(
    db_options: &DatabaseTableOptionsFactory,
) -> Options {
    db_options.pending_recover_slivers().to_options()
}

...
impl BlobInfoTable {
    ...
    pub(crate) fn process_pool_blob_registered(
        ...
        let change_info = PoolBlobChangeInfo { epoch: event.epoch };
    ...
    pub(crate) fn process_pool_blob_certified(
        ...
        let change_type = if event.is_extension {
            BlobStatusChangeType::Extend
        } else {
            BlobStatusChangeType::Certify
        };

        let pool_change_info = PoolBlobChangeInfo { epoch: event.epoch };
        ...
        // For extensions, skip per-object merge — pool extension doesn't change per-object state.
        // For certify, emit a per-object merge operand (end_epoch is unused by the Certify
        // handler but BlobStatusChangeInfo requires it, so we set it to 0).
...
#[derive(Debug, Deserialize, Serialize, PartialEq, Eq, Clone)]
pub(super) struct PoolBlobChangeInfo {
    pub(super) epoch: Epoch,
}
...
pub(super) enum BlobInfoMergeOperand {
    ...
    PoolChangeStatus {
        change_type: BlobStatusChangeType,
        change_info: PoolBlobChangeInfo,
        storage_pool_id: ObjectID,
    },

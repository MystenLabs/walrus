...
//!
//! A [`CompletionInstruction`] makes this explicit: the executor (or a startup resumption path)
...
//! superseded finds none and finishes without touching the node status. This replaces the
//! previous re-read-and-check pattern ("only flip the status if it is still X") with a
//! structural rule: a task can only perform a status change that the epoch-change logic has
//! pre-authorized and not revoked.
...
#[derive(Debug)]
pub(crate) struct CompletionInstruction {
...
/// The slot holding the pending [`CompletionInstruction`] of a long-running task.
pub(crate) type CompletionSlot = Slot<CompletionInstruction>;

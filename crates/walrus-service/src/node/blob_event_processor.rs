...
impl BlobEventProcessor {
    ...
    pub async fn process_event(
        ...
        if let BlobEvent::Registered(event) = &blob_event {
            ...
            if let Err(error) = self.node.flush_pending_metadata(&event.blob_id).await {
                tracing::error!(
                    ?error,
                    blob_id = %event.blob_id,
                    "failed to persist pending metadata after blob registration"
                );
            }
            if let Err(error) = self.node.flush_pending_slivers(&event.blob_id).await {
                tracing::error!(
                    ?error,
                    blob_id = %event.blob_id,
                    "failed to persist pending slivers after blob registration"
                );
            }
            event_handle.mark_as_complete();
            return Ok(());

...
pub enum ComputeStorageConfirmationError {
    ...
    /// The storage node is out of disk space and cannot persist the pending upload.
    #[error(transparent)]
    #[rest_api_error(delegate)]
    OutOfSpace(#[from] OutOfSpace),

    /// Persisting the pending upload did not complete within the configured timeout.
    #[error(transparent)]
    #[rest_api_error(delegate)]

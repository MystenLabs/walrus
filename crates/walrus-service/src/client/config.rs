...
impl Config {
    ...
    /// Returns the sanitized symbol arena configuration for this client.
    pub fn symbol_arena_config(&self) -> SymbolArenaConfig {
        self.symbol_arena.to_core_config()
    }
}

/// User-facing overrides for the symbol arena session.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq, Eq)]
#[serde(default)]
pub struct SymbolArenaClientConfig {
    /// Maximum bytes reserved inside the arena; defaults to 4 GiB.
    pub capacity_bytes: Option<usize>,
    /// Spill budget that can overflow into temporary buffers.
    pub spill_bytes: Option<usize>,
}

impl Default for SymbolArenaClientConfig {
    fn default() -> Self {
        Self {
            capacity_bytes: None,
            spill_bytes: None,
...
impl SymbolArenaClientConfig {
    fn to_core_config(&self) -> SymbolArenaConfig {
        let mut config = SymbolArenaConfig::default();
        if let Some(capacity) = self.capacity_bytes {
            config.capacity_bytes = capacity;
        }
        if let Some(spill) = self.spill_bytes {
            config.spill_capacity_bytes = spill;
        }
        config
    }
}

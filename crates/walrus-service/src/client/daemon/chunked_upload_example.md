...
The chunked upload middleware enables resumable uploads of large blobs to the Walrus publisher daemon. This allows clients to upload large files reliably, with the ability to resume interrupted uploads.

## How it Works

The middleware intercepts requests with specific chunked upload headers, accumulates chunks until a complete blob is received, then forwards the reconstructed blob to the standard `put_blob` handler.

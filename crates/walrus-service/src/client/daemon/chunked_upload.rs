...
pub struct ChunkedUploadConfig {
    ...
    /// Maximum total size for a single upload
...
impl ChunkedUploadState {
    ...
    async fn store_chunk(
        ...
        let chunk_offset = u64::from(chunk_index) * self.config.max_chunk_size;
...
impl IntoResponse for ChunkedUploadError {
    fn into_response(self) -> Response {
        let (status, message) = match self {
            ChunkedUploadError::SessionNotFound => {
                (StatusCode::NOT_FOUND, "Upload session not found")
            }
            ChunkedUploadError::SessionMismatch => (
                StatusCode::BAD_REQUEST,
                "Upload session parameters mismatch",
            ),
            ChunkedUploadError::TotalSizeTooLarge => (
                StatusCode::PAYLOAD_TOO_LARGE,
                "Total upload size exceeds limit",
            ),
            ChunkedUploadError::ChunkTooLarge => {
                (StatusCode::PAYLOAD_TOO_LARGE, "Chunk size exceeds limit")
            }
            ChunkedUploadError::IncompleteUpload => {
                (StatusCode::BAD_REQUEST, "Upload is incomplete")
            }
...
mod tests {
    ...
    async fn test_chunked_upload_session_creation() {
        ...
        let session = state
            .get_or_create_session(session_id, 3, 1000)
            .await
            .unwrap();
    ...
    async fn test_chunk_storage_and_completion() {
        ...
        // Create session
        state
            .get_or_create_session(session_id, total_chunks, total_size)
            .await
            .unwrap();
    ...
    async fn test_background_cleanup_task() {
        ...
        let _session = state
            .get_or_create_session(session_id, 2, 200)
            .await
            .unwrap();

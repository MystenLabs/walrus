...
fn apply_upload_mode_to_config(
    ...
    if let Some(preset) = upload_mode {
        // With CLI presence, always override config-file values.
        config.communication_config = preset.apply_to(config.communication_config.clone(), true);
    }
    config
}
...
impl ClientCommandRunner {
    ...
    pub(crate) async fn store(
        ...
        config = apply_upload_mode_to_config(config, upload_mode);

...
pub struct TestCluster {
    ...
    /// Background event processor task handles
    pub event_processor_handles: Vec<JoinHandle<anyhow::Result<()>>>,
}
...
pub struct TestClusterBuilder {
    ...
    event_processor_handles: Vec<Option<JoinHandle<anyhow::Result<()>>>>,
}
...
impl TestClusterBuilder {
    ...
    /// Sets the event processor task handles for each storage node.
    pub fn with_event_task_handles(
        mut self,
        event_handles: Vec<JoinHandle<anyhow::Result<()>>>,
    ) -> Self {
        self.event_processor_handles = event_handles.into_iter().map(Some).collect();
        self
    }
    ...
    pub async fn build(self) -> anyhow::Result<TestCluster> {
        ...
        Ok(TestCluster {
            ...
            event_processor_handles: self.event_processor_handles.into_iter().flatten().collect(),
        })
    }
}
...
impl Default for TestClusterBuilder {
    fn default() -> Self {
        ...
        Self {
            ...
            event_processor_handles: shard_assignment.iter().map(|_| None).collect(),
